...
ALLOW = [
    ...
    f"/opt/bin/{R} status",
    f"{R} ssh --service postgres -- ls /var/lib/postgresql/data",
    f"{R} connect postgres",
    f"{R} api 'query {{ me {{ name }} }}'",
    f"{R} volume list",
    f"{R} volume backup list",
    f"{R} deployment logs 0123abcd",
    "timeout -k 5 30 cargo test",
    "timeout --signal=9 30 cargo test",
    f"{R} api 'mutation {{ volumeInstanceBackupCreate(volumeInstanceId: \"fa94c980-7737-46b3-be7d-801063c1322d\") }}'",
    "git status",
...
BLOCK = [
    ...
    f"{R} service foo web",  # an unknown group verb fails closed
    f"{R} api 'mutation {{ volumeDelete(id: \"x\") }}'",
    f"{R} api '\nmutation {{ volumeDelete(id: \"x\") }}'",
    f"{R} api 'mutation {{ volumeInstanceBackupCreate(volumeInstanceId: \"fa94c980-7737-46b3-be7d-801063c1322d\") volumeDelete(id: \"x\") }}'",
    f"{R} api 'mutation {{ volumeInstanceBackupCreate(volumeInstanceId: \"fa94c980-7737-46b3-be7d-801063c1322d\") }} mutation {{ volumeDelete(id: \"x\") }}'",
    f"{R} api 'query {{ me {{ name }} }}\nmutation {{ volumeDelete(id: \"x\") }}'",
    f"{R} domain add x",
    ...
    f"timeout -k 5 30 {R} redeploy",
    f"timeout --signal=9 30 {R} redeploy",
    "git --config-env FOO=BAR push --force",
    f"env FOO=1 {R} redeploy",
    f"xargs {R} redeploy",
    f"x={R}; $x redeploy",
    "x=rail; y=way; $x$y redeploy",
    "$HOME/bin/tool redeploy",
    f"echo '{R} redeploy' | bash",
    "echo 'cargo test' | bash",
    f"bash <<'EOF'\n{R} redeploy\nEOF",
