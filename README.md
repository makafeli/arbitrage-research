...
**Current implementation: captured research, durable virtual accounts, collection diagnostics and frozen exports.** The Rust workspace connects bounded same-chain route calculations to immutable input captures, PostgreSQL decision history and authenticated dashboard queries. Paper accounts retain exact balances and journals across restarts. Registered collection attempts retain provider/evaluation failures and unresolved outcomes separately from decisions. Session JSON/CSV exports share a consistent database snapshot with exact amounts, source counts and explicit missing-input limits. The dashboard has explicit Demo and Connected modes. Current protocol/provider qualification, complete atomic transaction simulation, automatic paper execution and live execution remain unfinished. See the [collection/export verification record](docs/18-COLLECTION-AND-EXPORT-VERIFICATION.md) for the exact tested checkpoint and remaining gates.
