...
Current acquisition records each pool independently; the engine rejects differing full block/slot contexts. A shared multi-pool acquisition anchor and deployed provider qualification are still required. Local batch age measures acquisition/processing elapsed time, not the age of finalized chainstate. Raw capture readiness therefore does not establish current-market freshness, complete quote readiness or an executable transaction. Replay labels user-supplied historical timing as modeled and never fetches current state to repair missing history.
