...
The final restore step opens `database.dump` without following symlinks, recomputes its size and SHA-256 against the externally anchored manifest, rewinds that same descriptor, and supplies it to `pg_restore` through stdin. The child receives no archive pathname to reopen. The internal command helper accepts only an open, bounded, read-only binary regular file positioned at byte zero; other commands retain closed stdin. The archive is streamed with bounded memory rather than loaded in full.

Replacing the pathname after the full bundle check but before descriptor hashing is refused before `pg_restore`. Replacing it after hashing cannot redirect the child to the new file: the original open descriptor is retained. A subsequently detected file mutation still fails the operation and requires quarantine, even when `pg_restore` already committed the original verified archive. This is not a guarantee against same-owner **in-place** writes to the open inode during execution; keep all bundle roots private, trusted and immutable throughout. Descriptor binding does not turn checksum integrity into trust in source database code.
...
The suite contains the original 24 offline cases, 10 diagnostic regressions and seven descriptor-input/replacement regressions (41 total). Database calls in the offline suite are mocked; pipe/deadline/diagnostic cases launch real local test subprocesses. This does not establish database integration. Recovery review separately runs the real project migrations on disposable PostgreSQL 17.11 and verifies synthetic storage rows, sequences, exact 24-digit values, pending commands, capture expiry bytes, audit triggers, uniqueness and actual private diagnostics for failing `psql`, `pg_dump` and `pg_restore`. Two additional real-database cases replace the dump after final verification and at dispatch: the first leaves an empty target, and the second proves that the real `pg_restore` consumes only the original checked descriptor while the operation still fails and quarantines the changed bundle. Missing prerequisites fail instead of skip.
