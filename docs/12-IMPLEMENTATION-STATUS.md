...
| Research diagnostics/export | Durable pre-I/O collection attempts, typed terminal failures, unique decision associations, consistent bounded session JSON/CSV snapshots with redacted accounting projection | Registered attempts do not prove schedule completeness; raw capture files/expiry and asset decimals remain unverified/unavailable; full alerts and fair comparison summaries remain open |
