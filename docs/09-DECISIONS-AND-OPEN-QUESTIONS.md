...
| ADR-03 | Private, self-hosted single-operator product initially | Matches the personal research workflow and bounds access/custody complexity. | Confirmed commercial/multi-user use case; redesign authorization, isolation, quotas and custody before expansion. |
...
| Hosting | Linux private host; containerized services | Existing host resources, disk reliability, location, CPU contention, backup location and measured RTT. |
...
| Existing host and preferred region? | Linux self-hosted container-capable environment | Selecting feed/provider deployment regions |
