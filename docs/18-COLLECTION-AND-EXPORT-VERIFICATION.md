...
The accepted runtime checkpoint is `586bf6f6d2b796beb9aee95263fc97feef0e85fd` in [PR #90](https://github.com/makafeli/arbitrage-research/pull/90). [Run 34723924678](https://github.com/makafeli/arbitrage-research/actions/runs/34723924678) passed all four jobs: **225 Rust tests, including all 52 mandatory real PostgreSQL tests**, with zero failures, ignored or filtered tests; **36 Chromium scenarios**; **25 Node tests**; the independent Uniswap reference reproduction; **21 importer regressions**, **31 negative contract cases** and **23 API operations**; all three Railway container builds and Caddy validation. The actual shipped TypeScript client also passed against the Rust HTTP service and PostgreSQL, including scoped collection queries, session creation without worker start, frozen exports and repeated canonical content hashes.

The runtime suite verifies provider failure and process-loss outcomes, late STOP fencing, exact observation associations, per-route context rejections, repeatable-read consistency under a concurrent writer, row/byte refusal before partial export, redacted exact paper-account replay and HTTP scope/authentication. Source-based review matched the 52 PostgreSQL test names individually to successful CI log lines. These fixtures establish controlled engineering behavior; they do not qualify real providers or prove market profitability.

Local Rust 1.91.1 compilation, formatting and strict Clippy passed. The selected changed-component local runtime cohorts passed 22 API, four storage and four worker tests; database tests were deliberately left to CI. The canonical successful run uses the committed Rust 1.90 and PostgreSQL 17.11 environment. The previous accepted baseline, PR #89, had 210 Rust/41 PostgreSQL and 30 browser tests. This cohort adds 15 Rust tests (11 requiring PostgreSQL), six browser scenarios and five Node tests.

The evidence/documentation follow-up retains this runtime checkpoint. Its own final PR-head checks and merge result are recorded on GitHub; no self-referential commit hash is inferred here.

## Research denominators

The first integrated run on `c02e6cabc26f607343ffcc286d38fbc6d3f3cff1` ([34723515064](https://github.com/makafeli/arbitrage-research/actions/runs/34723515064)) passed specifications, all 36 browsers, 25 Node tests and the three container builds. Rust reported 224 passed and one failed, with zero ignored/filtered tests; the separate real-client smoke did not run after that failure. The new different-context process test incorrectly expected a whole-batch evaluation error. Existing engine behavior correctly preserves two `REJECTED` observations with `CAPTURE_CONTEXT_MISMATCH`. The corrected test retains its 20-second deadline and verifies those exact rejections, zero quoted opportunities and no misclassification as provider or fatal evaluation failure. Production behavior was not changed. The successful complete follow-up is the accepted runtime checkpoint above.
