...
| Experiments + Strategies + Real | `components/SessionCreation.tsx` (gemount vanuit `ConnectedApp.tsx`), `pages/StrategiesPage.tsx`, `components/RealTradingPanel.tsx` | Experiments: `ol.steps` 01 Validated configuration · 02 Session network + Experiment reference · 03 review + create (labels unchanged); Strategies: `.listrow` list → `table.research-table` (Mode · Networks · Digest (mono) · Strategies · IMMUTABLE pill); Real: panel restyle only |
