...
async fn pending_commit_child_entry() {
    ...
    let pending = store
        .issue_command(&operator, &session, "same-intent", start_intent())
        .await
        .unwrap();
...
async fn killed_pending_process_preserves_intent_and_recovers_stopped_on_both_chains() {
    for network in ["base-mainnet", "solana-mainnet"] {
        for mode in ["OBSERVE", "PAPER"] {
            ...
            store
                .save_configuration(&operator, "synthetic:config", json!({"mode":mode}))
                .await
                .unwrap();
            let session = store
                .create_session(
                    ...
                    NewSession {
                        ...
                        experiment_id: "synthetic:process-loss".into(),
            ...
            let child = Command::new(std::env::current_exe().unwrap())
                ...
                .env(CHILD, "1")
                .env("ARB_TEST_OPERATOR", &operator)
                .env("ARB_TEST_SESSION", &session.session_id)
                .env("ARB_TEST_NETWORK", network)
                .env("ARB_TEST_MARKER", &marker)
                .stdin(Stdio::null())
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .spawn()
            ...
            let command = loop {
                assert!(
                    guard.process.try_wait().unwrap().is_none(),
                    "crash child exited before marker"
                );
            ...
            let replacement = ControlWorker::claim(
                ...
                &session.session_id,
                network,
                "replacement-after-process-loss",
                60,
            )
            .await
            .unwrap();
            ...
            assert_eq!(
                replacement.tick(true).await.unwrap().session.observed_state,
                "STOPPED"
            );
            let recorded: i64 =
                sqlx::query_scalar("SELECT count(*) FROM control_commands WHERE session_id=$1")
