...
fn initialize(snapshot: &mut PoolSnapshot, registry: &PoolRegistry, tick: i32, gross: u128, net: i128) {
    ...
    let value = U256::from_str_radix(snapshot.bitmap_words[&index].trim_start_matches("0x"), 16).unwrap();
    snapshot.bitmap_words.insert(index, format!("0x{:064x}", value | (U256::one() << bit)));
    snapshot.initialized_ticks.insert(tick, TickState {
...
fn tick_math_golden_boundaries_and_integer_inverse() {
    ...
    assert_eq!(sqrt_at_tick(MAX_TICK).unwrap().to_string(),
        "1461446703485210103287273052203988822378723970342");
    assert_eq!(sqrt_at_tick(-1).unwrap().to_string(), "79224201403219477170569942574");
    assert_eq!(sqrt_at_tick(1).unwrap().to_string(), "79232123823359799118286999568");
    for tick in [MIN_TICK, -524288, -2560, -1, 0, 1, 2560, 524288, MAX_TICK - 1] {
...
fn full_width_multiply_divide_rounding_and_overflow() {
    ...
    assert_eq!(mul_div(U256::from(5), U256::from(3), U256::from(2), false).unwrap(), U256::from(7));
    assert_eq!(mul_div(U256::from(5), U256::from(3), U256::from(2), true).unwrap(), U256::from(8));
...
fn exact_constant_liquidity_quotes_include_fees_in_both_directions() {
    ...
    for direction in [true, false] {
        let quote = quote_exact_input_math(&snapshot, &registry, AtomicAmount::from(10_000), direction).unwrap();
        ...
        assert_eq!(quote.input_asset, if direction { registry.token0.clone() } else { registry.token1.clone() });
    }
...
fn negative_tick_compression_and_empty_word_boundaries_are_exact() {
    let mut words = BTreeMap::from([(-2, U256::zero()), (-1, U256::one() << 255), (0, U256::one())]);
...
fn crossing_exact_current_boundary_obeys_snapshot_tick_side() {
    ...
    let after = quote_exact_input_math(&already_crossed, &registry, AtomicAmount::from(10_000), true).unwrap();
...
fn full_input_is_required_and_extreme_inputs_terminate_at_both_window_edges() {
    ...
    for direction in [true, false] {
        let error = quote_exact_input_math(&snapshot, &registry, huge.clone(), direction).unwrap_err();
        assert!(error.0.contains("captured"), "{}", error.0);
    }
    let (edge, narrow_registry) = fixture(0, 1_000_000_000_000, 0, 0);
    assert!(quote_exact_input_math(&edge, &narrow_registry, AtomicAmount::from(10_000), true).is_err());
...
fn liquidity_crossing_underflow_and_overflow_fail_closed() {
    ...
    let error = quote_exact_input_math(&lower, &registry, AtomicAmount::from(100), true).unwrap_err();
    assert_eq!(error.0, "Uniswap V3 liquidity crossing overflows or underflows");

    let (mut higher, registry) = fixture(0, u128::MAX, -1, 1);
    initialize(&mut higher, &registry, 10, 1, 1);
    let error = quote_exact_input_math(&higher, &registry, u128::MAX.to_string().parse().unwrap(), false).unwrap_err();
    assert_eq!(error.0, "Uniswap V3 liquidity crossing overflows or underflows");

    let (mut reverse, registry) = fixture(0, 1000, -1, 1);
    initialize(&mut reverse, &registry, 10, 2000, -2000);
    let error = quote_exact_input_math(&reverse, &registry, AtomicAmount::from(100), false).unwrap_err();
    assert_eq!(error.0, "Uniswap V3 liquidity crossing overflows or underflows");
}
...
fn zero_liquidity_gap_is_traversed_without_unbounded_work() {
    ...
    let quote = quote_exact_input_math(&snapshot, &registry, AtomicAmount::from(10), false).unwrap();
...
fn all_bitmap_ticks_and_window_words_must_be_present_and_consistent() {
    ...
    assert!(quote_exact_input_math(&missing_tick, &registry, AtomicAmount::from(10), false).is_err());
    let mut extra_tick = snapshot.clone();
    extra_tick.initialized_ticks.insert(20, TickState { liquidity_gross: "100".into(), liquidity_net: "100".into() });
...
fn identity_schema_integer_and_price_validation_fail_closed() {
    ...
    for liquidity in ["01", "-1", "+1", "0", "340282366920938463463374607431768211456"] {
    ...
    wrong_price.tick = -2;
    assert!(quote_exact_input_math(&wrong_price, &registry, AtomicAmount::from(10), false).is_err());
    ...
    assert!(quote_exact_input_math(&snapshot, &unknown_schema, AtomicAmount::from(10), false).is_err());
    let mut unsupported_spacing = registry.clone();
    unsupported_spacing.tick_spacing = 16384;
    assert!(quote_exact_input_math(&snapshot, &unsupported_spacing, AtomicAmount::from(10), false).is_err());
}
...
fn tick_liquidity_canonical_net_and_protocol_cap_are_validated() {
    ...
    for net in ["-0", "+1", "01", "-01", "101", "-101", "-170141183460469231731687303715884105728"] {
        let mut bad = snapshot.clone();
        bad.initialized_ticks.get_mut(&10).unwrap().liquidity_net = net.into();
        assert!(quote_exact_input_math(&bad, &registry, AtomicAmount::from(10), false).is_err());
    }
    snapshot.initialized_ticks.get_mut(&10).unwrap().liquidity_gross = u128::MAX.to_string();
...
fn protocol_price_limits_reject_partial_input() {
    for direction in [true, false] {
        let tick = if direction { MIN_TICK + 1 } else { MAX_TICK - 1 };
        let word = tick.div_euclid(10).div_euclid(256) as i16;
        let (snapshot, registry) = fixture(tick, 1_000_000_000_000, word, word);
        let error = quote_exact_input_math(&snapshot, &registry, u128::MAX.to_string().parse().unwrap(), direction).unwrap_err();
        assert!(error.0.contains("captured"), "{}", error.0);
    }
}
...
fn next_sqrt_uint256_overflow_fallback_is_exact_and_bounded() {
    ...
    let expected = numerator / divisor + if (numerator % divisor).is_zero() { U256::zero() } else { U256::one() };
...
fn next_sqrt_matches_pinned_official_sdk_primitive_vectors() {
    let reference: serde_json::Value = serde_json::from_str(include_str!("../../tests/reference/golden.json")).unwrap();
    let primitives = reference["primitives"].as_array().unwrap();
    assert!(!primitives.is_empty(), "official primitive vectors must not be empty");
    for vector in primitives {
        assert_eq!(vector["operation"].as_str().unwrap(), "next_sqrt_from_input");
        ...
        assert_eq!(next_sqrt_from_input(price, liquidity, amount, direction).unwrap(),
            expected, "{}", vector["name"].as_str().unwrap());
    }
}
