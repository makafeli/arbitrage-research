...
fn config(mode: &str, starting_asset: &str) -> ValidatedConfig {
    let inert = ValidatedConfig::from_toml(include_str!("../../../config/research.example.toml"))
        .unwrap();
...
fn changed_starting_asset_and_mode_require_stopped_and_keep_old_snapshot_intact() {
    ...
    for replacement_config in [config("PAPER", B), config("OBSERVE", A), config("REPLAY", B)] {
        for state in [
            State::Recovering, State::Running, State::Pausing, State::Paused,
            State::Draining, State::Faulted,
        ] {
            let error = session.replacement(state, &replacement_config, NetworkId::BaseMainnet)
                .unwrap_err();
            assert_eq!(error.field, "session.state");
        }
        let replacement = session.replacement(
            State::Stopped, &replacement_config, NetworkId::BaseMainnet,
        ).unwrap();
        assert_eq!(replacement.configuration_digest(), replacement_config.digest());
        assert_eq!(replacement.starting_asset(), replacement_config.starting_asset(NetworkId::BaseMainnet));
        assert_ne!(replacement.configuration_digest(), session.configuration_digest());
...
fn persisted_effective_configuration_keeps_exact_units_and_set_canonicalization() {
    ...
    reordered["networks"]["base"]["verified_pool_ids"].as_array_mut().unwrap().reverse();
    reordered["research"]["trade_sizes_minor"].as_array_mut().unwrap().reverse();
...
fn invalid_effective_configuration_errors_never_echo_inline_secrets() {
    let original = config("PAPER", A);
    for pointer in ["/networks/base/rpc_secret_reference", "/storage/database_secret_reference"] {
        let mut value: Value = serde_json::from_str(original.effective_json()).unwrap();
        *value.pointer_mut(pointer).unwrap() = json!("https://user:NEVER_EXPORT_THIS@example.invalid");
