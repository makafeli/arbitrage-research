...
pub enum PoolState {
    Base {
        snapshot: arb_evm::PoolSnapshot,
        registry: arb_evm::PoolRegistry,
    },
    Solana {
        snapshot: arb_solana::PoolSnapshot,
        registry: arb_solana::PoolRegistry,
    },
}
#[derive(Clone, Debug)]
...
pub trait EvaluationGate {
    fn state(&self) -> GateState;
}
impl EvaluationGate for GateState {
    fn state(&self) -> GateState {
        *self
    }
}
...
fn error(code: &'static str) -> EngineError {
    EngineError { code }
}
...
pub fn capabilities(network_id: NetworkId) -> CapabilityReport {
    CapabilityReport {
        network_id,
        venue_family: venue(network_id),
        read_decode_implemented: true,
        research_math_implemented: true,
        qualified_quote: false,
        transaction_build: false,
        full_transaction_simulation: false,
        submit: false,
        reason_codes: vec![
...
impl PoolState {
    pub fn network_id(&self) -> NetworkId {
        match self {
            Self::Base { .. } => NetworkId::BaseMainnet,
            Self::Solana { .. } => NetworkId::SolanaMainnet,
        }
    }
    fn identities(&self) -> Result<(PoolId, AssetId, AssetId), EngineError> {
        ...
        let (pool, a, b) = match self {
            Self::Base { snapshot, registry } => {
                registry
                    .validate()
                    .map_err(|_| error("CAPTURE_IDENTITY_MISMATCH"))?;
                if !snapshot.pool.eq_ignore_ascii_case(&registry.pool) {
                    return Err(error("CAPTURE_IDENTITY_MISMATCH"));
                }
                (
                    snapshot.pool.as_str(),
                    registry.token0.as_str(),
                    registry.token1.as_str(),
                )
            }
            Self::Solana { snapshot, registry } => {
                registry
                    .validate()
                    .map_err(|_| error("CAPTURE_IDENTITY_MISMATCH"))?;
                if snapshot.pool != registry.pool
                    || snapshot.state.mint_a != registry.mint_a
                    || snapshot.state.mint_b != registry.mint_b
                    || snapshot.state.vault_a != registry.vault_a
                    || snapshot.state.vault_b != registry.vault_b
                    || snapshot.state.whirlpools_config != registry.whirlpools_config
                {
                    return Err(error("CAPTURE_IDENTITY_MISMATCH"));
                }
                (
                    snapshot.pool.as_str(),
                    registry.mint_a.as_str(),
                    registry.mint_b.as_str(),
                )
            }
        };
        Ok((
            PoolId::new(network, pool).map_err(|_| error("CAPTURE_IDENTITY_MISMATCH"))?,
    ...
    fn observed_at_ms(&self) -> u64 {
        match self {
            Self::Base { snapshot, .. } => snapshot.quality.observed_at_ms,
            Self::Solana { snapshot, .. } => snapshot.quality.observed_at_ms,
        }
    }
    fn context(&self) -> &StateContext {
        match self {
            Self::Base { snapshot, .. } => &snapshot.context,
            Self::Solana { snapshot, .. } => &snapshot.context,
        }
    }
}
struct PoolView<'a> {
    input: &'a CapturedPool,
    id: PoolId,
    a: AssetId,
    b: AssetId,
}
    ...
    fn quote(
        ...
        match pool {
            PoolState::Base { snapshot, registry } => {
                let q = arb_evm::math::quote_exact_input_math(
                    snapshot,
                    registry,
                    input.clone(),
                    asset.address().eq_ignore_ascii_case(&registry.token0),
                )
            ...
            PoolState::Solana { snapshot, .. } => {
                let value = input
                    .to_string()
                    .parse::<u64>()
                    .map_err(|_| error("UNSUPPORTED_SIZE"))?;
                let q = arb_solana::math::quote_exact_input_math(
                    snapshot,
                    value,
                    asset.address() == snapshot.state.mint_a,
                )
                .map_err(|failure| map_math_error(failure.0))?;
                ...
                Ok(LegQuote {
                    output: q
                        .amount_out
                        .parse()
                        .map_err(|_| error("ARITHMETIC_OVERFLOW"))?,
                    fee: q
                        .pool_fee_in_input_asset
                        .parse()
                        .map_err(|_| error("ARITHMETIC_OVERFLOW"))?,
                })
            }
...
fn map_math_error(message: &'static str) -> EngineError {
    ...
    let code = if message.contains("coverage")
        || message.contains("window")
        || message.contains("tick array")
        || message.contains("bitmap")
        || message == "input exceeds captured Uniswap V3 price limit"
        || message.contains("no captured") && message.contains("price range")
    {
        "INCOMPLETE_TICK_COVERAGE"
    } else if message.contains("overflow") {
        "ARITHMETIC_OVERFLOW"
    } else if message.contains("zero output") || message.contains("output rounds to zero") {
...
fn check_gate(
    ...
    if !state.admission_open || state.generation != request.generation {
        return Err(error("WORK_GENERATION_CANCELLED"));
    }
    if state.now_monotonic_ms >= request.deadline_monotonic_ms {
        return Err(error("DEADLINE_EXPIRED"));
    }
    state
        .now_monotonic_ms
        .checked_sub(request.observed_monotonic_ms)
        .ok_or_else(|| error("STALE_INPUT"))
}
...
fn base_trace(
    ...
    DecisionTrace {
        ...
        source_kind: request.dataset_origin.source_kind(),
...
fn coverage(
    ...
    code: &'static str,
    no_route: bool,
) -> Result<DecisionTrace, EngineError> {
    let result = if no_route && !refs.is_empty() {
        DecisionResult::NoRoute {
...
fn route_trace(
    ...
    trace.route = vec![
        DecisionLeg {
            pool_id: route.first.id.clone(),
            asset_in: route.start.clone(),
            asset_out: route.middle.clone(),
            venue_family: venue(request.network_id).into(),
        },
        DecisionLeg {
            pool_id: route.second.id.clone(),
            asset_in: route.middle.clone(),
            asset_out: route.start.clone(),
            venue_family: venue(request.network_id).into(),
        },
    ];
    trace.diagnostics = vec![
        "RESEARCH_MATH_ONLY".into(),
        "CURRENT_PROTOCOL_EQUIVALENCE_UNQUALIFIED".into(),
        "TOKEN_BEHAVIOR_UNQUALIFIED".into(),
        "EXTERNAL_COSTS_UNAVAILABLE".into(),
        "FULL_TRANSACTION_SIMULATION_NOT_RUN".into(),
...
fn evaluate_inner(
    ...
    for pool in request.pools {
        ...
        let latest = request
            .observed_at_unix_ms
            .checked_add(age)
            .ok_or_else(|| error("CAPTURE_CONTEXT_MISMATCH"))?;
        if pool.state.observed_at_ms() < request.observed_at_unix_ms
            || pool.state.observed_at_ms() > latest
        {
            return Err(error("CAPTURE_CONTEXT_MISMATCH"));
        }
        if pool.configuration_digest != request.configuration.digest() {
            return Err(error("CAPTURE_CONFIGURATION_MISMATCH"));
        }
        if pool.origin != request.dataset_origin {
            return Err(error("CAPTURE_ORIGIN_MISMATCH"));
        }
        if pool.state.network_id() != request.network_id {
            return Err(error("CAPTURE_IDENTITY_MISMATCH"));
        }
        match &pool.state {
            PoolState::Base { snapshot, .. }
                if !matches!(snapshot.context, StateContext::Evm { .. }) =>
            {
                return Err(error("CAPTURE_CONTEXT_MISMATCH"));
            }
            PoolState::Solana { snapshot, registry } => {
                if registry.expected_genesis_hash
                    != request.configuration.expected_genesis_identity()
                    || !matches!(&snapshot.context,StateContext::Solana {genesis_hash,..} if genesis_hash==&registry.expected_genesis_hash)
                    || snapshot
                        .tick_arrays
                        .iter()
                        .any(|array| !registry.tick_arrays.contains(&array.address))
                {
                    return Err(error("CAPTURE_CONTEXT_MISMATCH"));
                }
            }
        ...
        if !request
            .configuration
            .verified_pools(request.network_id)
            .contains(&id)
            || !request
                .configuration
                .verified_assets(request.network_id)
                .contains(&a)
            || !request
                .configuration
                .verified_assets(request.network_id)
        ...
        if !seen.insert(id.to_string()) {
            return Err(error("DISTINCT_POOL_REQUIRED"));
        }
    ...
    for first in &views {
        let middle = if &first.a == start {
            &first.b
        } else if &first.b == start {
            &first.a
        } else {
            continue;
        };
        for second in &views {
            if first.id != second.id
                && ((&second.a == middle && &second.b == start)
                    || (&second.b == middle && &second.a == start))
            {
        ...
        for amount in sizes {
                    ...
                    if a.block_time_seconds != b.block_time_seconds)
            {
                ...
                match first {
                    Ok(first) if !first.output.is_zero() => {
                        let second =
                            quoter.quote(&route.second.input.state, &first.output, &route.middle);
                        check_gate(request, gate)?;
            ...
            let result = match quoted {
                Ok((out, first_fee, second_fee)) if !out.is_zero() => DecisionResult::Quoted {
                    gross_delta_minor: SignedAmount::difference(&out, amount),
                    quoted_output_minor: out,
                    included_pool_fees: vec![first_fee, second_fee],
                },
                Ok(_) => DecisionResult::Rejected {
                    reason_codes: vec!["OUTPUT_ROUNDS_TO_ZERO".into()],
                },
                Err(failure) => DecisionResult::Rejected {
                    reason_codes: vec![failure.code.into()],
                },
            };
            ...
            let size = serde_json::to_vec(&trace)
                ...
                .len()
                + 1;
            if bytes + size > MAX_BATCH_BYTES - SUMMARY_BYTE_RESERVE {
                exhausted = true;
                break 'routes;
            }
            bytes += size;
...
mod tests {
    ...
    fn base_pool(n: u64) -> CapturedPool {
        ...
        let registry = arb_evm::PoolRegistry {
            ...
            qualification_reference: "synthetic math fixture".into(),
            bitmap_word_min: -1,
            bitmap_word_max: 1,
        };
        let snapshot = arb_evm::PoolSnapshot {
            ...
            context: StateContext::Evm {
                ...
                block_timestamp_seconds: 1_700_000_000,
            ...
            quality: SnapshotQuality {
                ...
                max_age_ms: 1000,
                reasons: vec!["synthetic-fixture".into()],
            },
        };
        let digest = format!("sha256:{n:064x}");
        CapturedPool {
            capture: DecisionCaptureRef {
                capture_id: format!("synthetic-{n}"),
                manifest_digest: digest.clone(),
                snapshot_id: digest,
            },
            origin: DatasetOrigin::ManuallyConstructed,
    ...
    fn config(pools: &mut [CapturedPool], sizes: &[u64]) -> ValidatedConfig {
        ...
        json["research"]["trade_sizes_minor"] =
            serde_json::json!(sizes.iter().map(ToString::to_string).collect::<Vec<_>>());
        json["networks"]["base"]["enabled"] = serde_json::json!(true);
        json["networks"]["base"]["verified_pool_ids"] = serde_json::json!(
            pools
                .iter()
                .map(|p| p.state.identities().unwrap().0.to_string())
                .collect::<Vec<_>>()
        );
        let mut assets = pools
            .iter()
            .flat_map(|p| {
                let (_, a, b) = p.state.identities().unwrap();
        ...
        json["networks"]["base"]["starting_asset_id"] =
            serde_json::json!(format!("base-mainnet:{}", address(1)));
        json["networks"]["base"]["rpc_secret_reference"] = serde_json::json!("env:BASE_RPC_URL");
        json["networks"]["base"]["registry_qualification_digest"] =
    ...
    fn request<'a>(
        ...
        pools: &'a [CapturedPool],
    ) -> EvaluationRequest<'a> {
        EvaluationRequest {
            session_id: "synthetic-session",
            experiment_id: "manual-pair",
            generation: 7,
    ...
    fn missing_tick_coverage_is_missing_evidence_not_zero_return() {
        ...
        assert!(
            result
                .iter()
                .all(|t| matches!(&t.result, DecisionResult::Rejected { .. }))
        );
        assert!(result.iter().all(|t| t.to_opportunity().unwrap().is_none()));
    }
    #[test]
    fn capacity_is_explicit_and_serialized_batch_is_bounded() {
        ...
        assert!(
            matches!(&result.last().unwrap().result,DecisionResult::DataUnavailable {reason_codes} if reason_codes==&["EVALUATION_BUDGET_EXHAUSTED"])
        );
    ...
    fn capture_timestamp_must_fit_the_original_batch_interval() {
        ...
        let config = config(&mut pools, &[100_000]);
        if let PoolState::Base { snapshot, .. } = &mut pools[1].state {
            snapshot.quality.observed_at_ms += 11;
        }
        assert_eq!(
            evaluate(&request(&config, &pools), &gate())
                .unwrap_err()
                .code,
            "CAPTURE_CONTEXT_MISMATCH"
        );
        let mut later = gate();
        later.now_monotonic_ms = 11;
        assert!(evaluate(&request(&config, &pools), &later).is_ok());
        let mut request = request(&config, &pools);
        request.observed_at_unix_ms += 1;
        assert_eq!(
            evaluate(&request, &later).unwrap_err().code,
            "CAPTURE_CONTEXT_MISMATCH"
        );
    }
    #[test]
    fn duplicate_pool_config_mismatch_and_origin_spoof_are_not_admitted() {
        let mut pools = vec![base_pool(3), base_pool(4)];
        let config = config(&mut pools, &[100_000]);
        let mut duplicate = pools.clone();
        duplicate[1] = duplicate[0].clone();
        assert_eq!(
            evaluate(&request(&config, &duplicate), &gate())
                .unwrap_err()
                .code,
            "DISTINCT_POOL_REQUIRED"
        );
        let mut bad = pools.clone();
        bad[0].configuration_digest = format!("sha256:{}", "b".repeat(64));
        assert_eq!(
            evaluate(&request(&config, &bad), &gate()).unwrap_err().code,
            "CAPTURE_CONFIGURATION_MISMATCH"
        );
        let mut bad = pools;
        bad[0].origin = DatasetOrigin::RecordedLive;
        assert_eq!(
            evaluate(&request(&config, &bad), &gate()).unwrap_err().code,
    ...
    impl EvaluationGate for ChangingGate {
        fn state(&self) -> GateState {
            self.state.get()
        }
    }
    struct CancellingQuoter<'a> {
        gate: &'a ChangingGate,
        deadline: bool,
    }
    ...
    fn generation_change_and_deadline_between_legs_discard_the_entire_batch() {
        ...
        for (deadline, code) in [
            (false, "WORK_GENERATION_CANCELLED"),
            (true, "DEADLINE_EXPIRED"),
        ] {
            let gate = ChangingGate {
                state: Cell::new(gate()),
            };
            let quoter = CancellingQuoter {
                gate: &gate,
                deadline,
            };
            assert_eq!(
                evaluate_with(&request(&config, &pools), &gate, &quoter)
                    .unwrap_err()
                    .code,
                code
            );
    ...
    #[test]
    fn named_static_math_failures_keep_precise_public_diagnostics() {
        assert_eq!(
            map_math_error("Uniswap V3 quote output rounds to zero").code,
            "OUTPUT_ROUNDS_TO_ZERO"
        );
        assert_eq!(
            map_math_error("input exceeds captured Uniswap V3 price limit").code,
            "INCOMPLETE_TICK_COVERAGE"
        );
        assert_eq!(
            map_math_error("no captured Uniswap V3 price range").code,
            "INCOMPLETE_TICK_COVERAGE"
        );
        assert_eq!(
            map_math_error("invalid quote tick liquidity").code,
            "MATH_INPUT_REJECTED"
        );
    }
    #[test]
    fn static_capabilities_cannot_be_promoted_by_configured_registry_or_quality_flags() {
        for network in [NetworkId::BaseMainnet, NetworkId::SolanaMainnet] {
            ...
            assert!(
                !capability.qualified_quote
                    && !capability.full_transaction_simulation
                    && !capability.transaction_build
                    && !capability.submit
            );
        }
        let mut pools = vec![base_pool(3), base_pool(4)];
        ...
        for pool in &mut pools {
            if let PoolState::Base { snapshot, .. } = &mut pool.state {
                ...
                snapshot.quality.coherent = true;
                snapshot.quality.complete_for_quote = true;
            }
        }
        for trace in evaluate(&request(&config, &pools), &gate()).unwrap() {
            assert_eq!(
    ...
    fn solana_pool(n: u8) -> CapturedPool {
        ...
        let registry = arb_solana::PoolRegistry {
            schema_version: 1,
            expected_genesis_hash: key(11),
            pool: key(n),
            whirlpools_config: key(10),
            program_data: key(9),
            program_data_sha256: format!("sha256:{}", "0".repeat(64)),
            mint_a: key(1),
            mint_b: key(2),
            decimals_a: 6,
            decimals_b: 6,
        ...
        let snapshot = arb_solana::PoolSnapshot {
            ...
            context: StateContext::Solana {
                ...
                account_context: "synthetic-single-response".into(),
            },
            state: arb_solana::WhirlpoolState {
                whirlpools_config: registry.whirlpools_config.clone(),
                tick_spacing: 64,
                fee_tier_index: 64,
                fee_rate: 500,
                protocol_fee_rate: 0,
                ...
                mint_a: registry.mint_a.clone(),
                mint_b: registry.mint_b.clone(),
                vault_a: registry.vault_a.clone(),
                vault_b: registry.vault_b.clone(),
            },
            tick_arrays: registry
                .tick_arrays
                .iter()
                .zip([-5632, 0, 5632])
                .map(|(address, start_tick_index)| arb_solana::FixedTickArray {
        ...
        CapturedPool {
            capture: DecisionCaptureRef {
                capture_id: format!("manual-solana-{n}"),
                manifest_digest: digest.clone(),
                snapshot_id: digest,
            },
            origin: DatasetOrigin::ManuallyConstructed,
    ...
    fn actual_solana_math_routes_are_candidate_only_and_large_input_is_rejected() {
        ...
        let mut json: serde_json::Value = serde_json::from_str(inert.effective_json()).unwrap();
        json["research"]["trade_sizes_minor"] =
            serde_json::json!(["1000000", "18446744073709551616"]);
        json["networks"]["solana"]["enabled"] = serde_json::json!(true);
        json["networks"]["solana"]["verified_pool_ids"] = serde_json::json!(
            pools
                .iter()
                .map(|p| p.state.identities().unwrap().0.to_string())
                .collect::<Vec<_>>()
        ...
        json["networks"]["solana"]["verified_asset_ids"] =
            serde_json::json!([a.to_string(), b.to_string()]);
        json["networks"]["solana"]["starting_asset_id"] = serde_json::json!(a.to_string());
        json["networks"]["solana"]["rpc_secret_reference"] =
            serde_json::json!("env:SOLANA_RPC_URL");
        json["networks"]["solana"]["registry_qualification_digest"] =
            serde_json::json!(format!("sha256:{}", "a".repeat(64)));
        json["networks"]["solana"]["expected_genesis_identity"] =
            serde_json::json!(bs58::encode([11; 32]).into_string());
        let config = ValidatedConfig::from_effective_json(&json.to_string()).unwrap();
        for pool in &mut pools {
            pool.configuration_digest = config.digest().into();
        }
        ...
        assert_eq!(
            traces
                .iter()
                .filter(|t| matches!(t.result, DecisionResult::Quoted { .. }))
                .count(),
            2
        );
