...
use arb_storage::{
    CaptureSourceBinding, IngestionBinding, IngestionCursor, IngestionHalt, IngestionHead,
};
use sqlx::PgPool;
...
fn refs(count: usize) -> Vec<DecisionCaptureRef> {
    (0..count)
        .map(|n| {
            ...
            DecisionCaptureRef {
                capture_id: format!("publication-{n}"),
...
fn new_publication_limit_keeps_legacy_validation_and_empty_diagnostics() {
    ...
    trace("synthetic-session", 1, 1003, "DATA_UNAVAILABLE")
...
async fn publication_rejects_ninth_reference_before_partial_batch_persistence() {
    ...
    assert!(matches!(
        store
            .append_decision_traces(&claim, generation, &[eight.clone(), nine])
            .await,
    ...
    let saved = store
        .append_decision_traces(&claim, generation, std::slice::from_ref(&eight))
        .await
        .unwrap();
    let repeated = store
        .append_decision_traces(&claim, generation, std::slice::from_ref(&eight))
...
async fn source_fixture(start: bool) -> SourceFixture {
    ...
    store
        .save_configuration(&operator, &hash("a"), json!({"mode":"OBSERVE"}))
        .await
        .unwrap();
    let id = store
        .create_session(
        ...
        .session_id;
    let claim = store
        .claim_worker(&operator, &id, "base-mainnet", "source-worker", 60)
        .await
        .unwrap();
    ...
    let source = CaptureSourceBinding {
        ...
        binding: IngestionBinding {
            schema_version: 1,
            network_id: "base-mainnet".into(),
            registry_digest: hash("b"),
            abi_source_commit: "c".repeat(40),
            dataset_origin: "MANUALLY_CONSTRUCTED".into(),
            pool_addresses: vec![format!("0x{:040x}", 3), format!("0x{:040x}", 4)],
        },
    };
    let initial = store
        .create_ingestion(&operator, "source", &source.binding, &head(100))
        .await
        .unwrap();
    let value = json!({
    ...
    let cursor = store
        .commit_ingestion(&operator, "source", &initial, value)
        .await
        .unwrap();
    store
        .configure_capture_ingestion_source(&claim, Some(&source))
        .await
        .unwrap();
    let generation = if start {
        ...
        for (id, digest) in [("capture-one", hash("1")), ("capture-two", hash("2"))] {
            store
                .record_capture_admission(&claim, generation, id, &digest, "/synthetic/source")
                .await
                .unwrap();
        }
        generation
...
async fn source_requirement_is_persisted_idempotent_and_cannot_be_removed_or_changed() {
    ...
    f.store
        .configure_capture_ingestion_source(&f.claim, Some(&f.source))
        .await
        .unwrap();
    assert!(matches!(
        f.store
            .configure_capture_ingestion_source(&f.claim, None)
            .await,
    ...
    assert!(
        f.store
            .configure_capture_ingestion_source(&f.claim, Some(&wrong))
            .await
            .is_err()
    );
    for sql in [
        "DELETE FROM session_ingestion_sources WHERE operator_id=$1",
        "UPDATE session_ingestion_sources SET stream_id='replacement' WHERE operator_id=$1",
    ] {
        assert!(
            sqlx::query(sql)
                .bind(&f.operator)
...
async fn source_requirement_guards_first_quote_then_exact_binding_publishes_and_halt_invalidates() {
    ...
    assert!(matches!(
        f.store
            .append_decision_traces(&f.claim, f.generation, std::slice::from_ref(&first))
            .await,
        Err(StoreError::Conflict(
            "capture source continuity unavailable"
        ))
    ));
    assert_eq!(
        f.store
            .decision_coverage(&f.operator, &f.id)
            .await
            .unwrap()
            .raw_observations,
        "0"
    ...
    let before = serde_json::to_value(
        f.store
            .get_decision_trace(&f.operator, &first.observation_id)
            .await
            .unwrap()
            .trace,
    )
    ...
    assert!(matches!(
        f.store
            .append_decision_traces(
                &f.claim,
                f.generation,
                &[trace(&f.id, f.generation, 1001, "QUOTED")]
            )
            .await,
        Err(StoreError::Conflict(
            "capture source continuity unavailable"
    ...
    f.store
        .append_decision_traces(
            &f.claim,
            f.generation,
            &[trace(&f.id, f.generation, 1002, "DATA_UNAVAILABLE")],
        )
        .await
        .unwrap();
}
...
async fn incomplete_mismatched_and_stale_bindings_never_persist_a_prefix() {
    ...
    assert!(
        f.store
            .bind_captures_to_ingestion(&f.claim, f.generation, &f.source, &head(101), &missing)
            .await
            .is_err()
    );
    ...
    for changed_origin in [false, true] {
        ...
        if changed_origin {
            source.binding.dataset_origin = "RECORDED_LIVE".into();
        } else {
            source.binding.registry_digest = hash("d");
        }
        assert!(
            f.store
                .bind_captures_to_ingestion(
...
async fn stopped_generation_cannot_add_source_dependencies() {
    ...
    f.store
        ...
        .unwrap();
    f.store
        .apply_pending(&f.claim, true, |_| Ok(()))
        .await
        .unwrap();
