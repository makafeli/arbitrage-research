...
async fn audit_count(f: &Fixture, stream: &str) -> i64 {
    sqlx::query_scalar("SELECT count(*) FROM ingestion_invalidations WHERE operator_id=$1 AND stream_id=$2")
        .bind(&f.operator).bind(stream).fetch_one(&f.pool).await.unwrap()
}
...
async fn every_terminal_reason_invalidates_prior_snapshots_without_rewriting_payloads() {
    ...
    for (reason, name) in [
        ...
        let current = f
            ...
            .await
            .unwrap();
        let before = f.store.ingestion_batches(&f.operator, name, 0, 16).await.unwrap();
        ...
        let after = f.store.ingestion_batches(&f.operator, name, 0, 16).await.unwrap();
        ...
        let audit = sqlx::query("SELECT * FROM ingestion_invalidations WHERE operator_id=$1 AND stream_id=$2")
            .bind(&f.operator).bind(name).fetch_one(&f.pool).await.unwrap();
        assert_eq!(audit.get::<String, _>("reason"), name);
        assert_eq!(audit.get::<String, _>("policy_version"), "base-finalized-stream-v1");
        assert_eq!(audit.get::<String, _>("source"), "STREAM_TRANSITION");
        assert_eq!(audit.get::<Value, _>("checkpoint"), json!(current.checkpoint));
...
async fn invalidation_survives_connection_restart_and_cannot_rearm() {
    ...
    let halted = f
        .store
        .halt_ingestion(&f.operator, "restart", &cursor, IngestionHalt::ProviderFailure)
        .await
        .unwrap();
    let reopened = Store::connect(&database()).await.unwrap();
    assert_eq!(reopened.ingestion_cursor(&f.operator, "restart").await.unwrap(), halted);
    assert!(reopened.commit_ingestion(&f.operator, "restart", &cursor, batch(&cursor)).await.is_err());
    let pool = PgPool::connect(&database()).await.unwrap();
    let error = sqlx::query_scalar::<_, Value>(GATE)
        .bind(&f.operator).bind("restart").bind(1_i64).bind(&hash)
        .fetch_one(&pool).await.unwrap_err();
...
async fn rejected_and_rolled_back_halts_leave_no_false_invalidation() {
    ...
    stale.revision = 0;
    assert!(f.store.halt_ingestion(&f.operator, "rollback", &stale, IngestionHalt::InvalidInput).await.is_err());
    assert_eq!(audit_count(&f, "rollback").await, 0);
    let mut tx = f.pool.begin().await.unwrap();
    sqlx::query(HALT).bind(&f.operator).bind("rollback").execute(&mut *tx).await.unwrap();
    ...
    assert_eq!(audit_count(&f, "rollback").await, 0);
    assert_eq!(f.store.ingestion_cursor(&f.operator, "rollback").await.unwrap(), cursor);
...
async fn audit_and_batch_history_reject_update_delete_and_false_insertion() {
    ...
    f.store.halt_ingestion(&f.operator, "immutable", &cursor, IngestionHalt::ContinuityLost).await.unwrap();
    for statement in [
        ...
        assert!(sqlx::query(statement).bind(&f.operator).execute(&f.pool).await.is_err());
    }
...
async fn exact_reference_and_missing_state_fail_with_fixed_errors() {
    ...
    for revision in [-1_i64, 0, 4097] {
        has_code(gate(&f, "references", revision, &hash).await.unwrap_err(), "22023");
    }
    has_code(gate(&f, "references", 2, &hash).await.unwrap_err(), "P0002");
    has_code(gate(&f, "missing", 1, &hash).await.unwrap_err(), "P0002");
    has_code(gate(&f, "references", 1, &format!("sha256:{}", "f".repeat(64))).await.unwrap_err(), "22023");
    let error = gate(&f, "references", 1, "PRIVATE_SENTINEL").await.unwrap_err();
    assert_eq!(error.as_database_error().unwrap().message(), "invalid snapshot reference");
    let rejected = sqlx::query_scalar::<_, Value>(GATE)
        .bind(Option::<String>::None).bind("references").bind(1_i64).bind(&hash)
        .fetch_one(&f.pool).await.unwrap_err();
...
async fn gate_retains_shared_lock_until_caller_transaction_commits() {
    ...
    let payload: Value = sqlx::query_scalar(GATE)
        .bind(&f.operator).bind("gate-first").bind(1_i64).bind(&hash)
    ...
    f.store.halt_ingestion(&f.operator, "gate-first", &cursor, IngestionHalt::ContinuityLost).await.unwrap();
...
async fn held_halt_excludes_gate_and_rollback_restores_eligibility() {
    ...
    let mut tx = f.pool.begin().await.unwrap();
    sqlx::query(HALT).bind(&f.operator).bind("halt-first").execute(&mut *tx).await.unwrap();
...
async fn concurrent_terminal_reports_leave_exactly_one_matching_audit_record() {
    ...
    let (first, second) = tokio::join!(
        f.store.halt_ingestion(&f.operator, "race", &cursor, IngestionHalt::ContinuityLost),
        f.store.halt_ingestion(&f.operator, "race", &cursor, IngestionHalt::ProviderFailure)
    );
...
async fn upgrade_records_old_halt_without_rewriting_its_detection_or_payload() {
    ...
    sqlx::raw_sql(&format!("CREATE SCHEMA {schema}; SET LOCAL search_path TO {schema}, pg_catalog"))
    ...
    sqlx::query(HALT).bind("upgrade").bind("old").execute(&mut *tx).await.unwrap();
    let detected: String = sqlx::query_scalar("SELECT updated_at::text FROM ingestion_streams WHERE operator_id='upgrade' AND stream_id='old'")
        .fetch_one(&mut *tx).await.unwrap();
    sqlx::raw_sql(include_str!("../../../migrations/0007_ingestion_invalidations.sql"))
        .execute(&mut *tx).await.unwrap();
    ...
    assert_eq!(view.get::<Value, _>("snapshot_checkpoint"), json!(head(101)));
    let retained: Value = sqlx::query_scalar("SELECT payload FROM ingestion_batches WHERE operator_id='upgrade' AND stream_id='old'")
        .fetch_one(&mut *tx).await.unwrap();
    assert_eq!(retained, json!({}));
    tx.rollback().await.unwrap();
    let remains: i64 = sqlx::query_scalar("SELECT count(*) FROM pg_namespace WHERE nspname=$1")
        .bind(schema).fetch_one(&f.pool).await.unwrap();
    assert_eq!(remains, 0);
}
