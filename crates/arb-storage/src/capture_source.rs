...
impl CaptureSourceBinding {
    pub fn validate(&self) -> Result<(), StoreError> {
        ...
        if self.stream_id.is_empty()
            ...
            || !self.stream_id.bytes().all(|c| c.is_ascii_alphanumeric() || b"-_.:".contains(&c))
        {
...
async fn check_source(
    ...
    let actual: IngestionBinding = serde_json::from_value(row.try_get("binding")?)
    ...
    if actual != expected.binding || row.try_get::<String, _>("state")? != "ACTIVE" {
        return Err(StoreError::Conflict("capture source binding or state differs"));
    }
    Ok(())
}
...
impl Store {
    ...
    pub async fn configure_capture_ingestion_source(
        ...
        if lifecycle(&session)?.state() != arb_domain::State::Stopped {
            return Err(StoreError::Conflict("capture source configuration requires stopped recovery"));
        }
        ...
        match source {
            None if existing.is_some() => return Err(StoreError::Conflict("configured capture source is required on restart")),
            None => {},
            Some(source) => {
                if claim.network_id != "base-mainnet" || existing.as_ref().is_some_and(|s| s != &source.stream_id) {
                    return Err(StoreError::Conflict("capture source differs from immutable session"));
                }
                ...
                if existing.is_none() {
                    ...
                    audit(&mut tx, &claim.session_id, &claim.operator_id, "CAPTURE_SOURCE_CONFIGURED", None,
                        json!({"stream_id":source.stream_id,"policy":"base-capture-continuity-v1"})).await?;
                }
            }
        }
        tx.commit().await?;
    ...
    pub async fn bind_captures_to_ingestion(
        ...
        source.validate()?;
        if captures.is_empty() || captures.len() > arb_domain::MAX_PUBLICATION_CAPTURE_REFS
        ...
        for capture in captures {
            ...
            if capture.capture_id.is_empty() || capture.capture_id.len() > 256
                || capture.capture_id.chars().any(char::is_control)
                || !unique.insert(&capture.capture_id)
                || capture.snapshot_id != capture.manifest_digest
                || !digest.is_some_and(|s| s.len()==64 && s.bytes().all(|b| b.is_ascii_digit() || (b'a'..=b'f').contains(&b)))
            {
                return Err(StoreError::InvalidInput("invalid capture source references"));
            }
        }
        let mut tx = self.pool.begin().await?;
        sqlx::query("SET LOCAL lock_timeout='2s'").execute(&mut *tx).await?;
        ...
        if !state.allows_evaluation() || state.generation() != generation || claim.network_id != "base-mainnet" {
        ...
        if configured.as_deref() != Some(source.stream_id.as_str()) {
            return Err(StoreError::Conflict("capture source is not configured for this session"));
        }
        check_source(&mut tx, &claim.operator_id, source).await?;
        let checkpoint = serde_json::to_value(checkpoint).map_err(|_| StoreError::InvalidInput("invalid capture checkpoint"))?;
        ...
        if batches.len() != 1 {
            return Err(StoreError::Conflict("captured checkpoint has no unique ingestion batch"));
        }
        ...
        let _: Value = sqlx::query_scalar("SELECT require_current_ingestion_snapshot($1,$2,$3,$4)")
            .bind(&claim.operator_id).bind(&source.stream_id).bind(revision).bind(&digest)
            .fetch_one(&mut *tx).await.map_err(continuity_error)?;
        let generation = i64::try_from(generation).map_err(|_| StoreError::InvalidInput("invalid capture generation"))?;
        ...
        for capture in captures {
            ...
            if !admitted { return Err(StoreError::Conflict("capture source batch contains an unadmitted capture")); }
