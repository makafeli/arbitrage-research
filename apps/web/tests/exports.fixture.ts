...
export function frozenExportFixture(): FrozenExport {
  ...
  const event = journal();
  return sealExport({ schema_version: '1.0.0', export_id: 'export-fixture-one', exported_at: '2026-09-12T12:01:00Z', content_sha256: exportHash,
    snapshot: { isolation: 'REPEATABLE_READ', scope: 'COMPLETE_STORED_SESSION', collection_completeness: 'UNKNOWN',
      source_counts: { decisions: '1', paper_runs: '1', paper_journal_events: '1', capture_catalog_entries: '1', collection_attempts: '3' } },
    methodology: { amounts: 'BASE_UNIT_INTEGER_STRINGS', asset_decimals: 'NOT_RETAINED_IN_DATABASE', costs: 'UNKNOWN_COSTS_REMAIN_NULL',
    ...
    data: { session: { session_id: 'session-paper', network_id: 'base-mainnet', mode: 'PAPER', observed_state: 'STOPPED', health: 'UNKNOWN',
      ...
      decisions: [trace], paper_runs: [{ run, journal: [{ event_id: event.event_id, recorded_at: event.recorded_at, source_payload_sha256: exportHash,
