...
import { AdapterSupport } from './components/AdapterSupport';
...
export function ConnectedApp({ light, onToggleTheme }: { light: boolean; onToggleTheme: () => void }) {
  ...
  return <>{!authenticated && <AccountAccess api={api} checking={authLoading} light={light} onToggleTheme={onToggleTheme} />}<div hidden={!authenticated}><a className="skip" href="#main">Skip to content</a><div className="shell">
    <aside className="sidebar"><div className="brand"><span className="brandmark" aria-hidden="true">↗</span>Arbitrage</div><div className="subbrand">TRADING WORKSPACE</div><nav className="trading-modes" aria-label="Trading mode"><button aria-pressed={tradingMode === 'paper'} disabled={unresolvedRequest} onClick={() => setTradingMode('paper')}>Paper trading</button><button aria-pressed={tradingMode === 'real'} disabled={unresolvedRequest} onClick={() => { setInspected(null); setTradingMode('real'); }}>Real trading</button></nav><p className="navlabel">{tradingMode === 'paper' ? 'Paper workspace' : 'Real workspace'}</p><nav className="nav" aria-label="Primary navigation" hidden={tradingMode !== 'paper'}>{pages.map((name, i) => <button key={name} aria-current={page === name ? 'page' : undefined} onClick={() => navigate(name)}><span className="navindex" aria-hidden="true">0{i + 1}</span>{name}</button>)}</nav><div className="sidefoot"><strong>Research first</strong>Persisted service evidence.<br />Dataset origins stay explicit.</div></aside>
    <div className="main"><header className="topbar"><div><strong>Private workspace</strong><small>Your trading account</small></div><div className="topactions"><span className="pill blue">{tradingMode === 'paper' ? 'PAPER TRADING' : 'REAL TRADING'}</span><button disabled={unresolvedRequest} onClick={() => setPasswordSettings(value => !value)}>Change password</button><button onClick={onToggleTheme} aria-label={`Switch to ${light ? 'dark' : 'light'} theme`}>{light ? 'Dark' : 'Light'} theme</button>{authenticated && <button disabled={unresolvedRequest} onClick={() => { void logout(); }}>Sign out</button>}</div></header>
      <main id="main" className="workspace">{passwordSettings && authenticated && <PasswordSettings api={api} onClose={() => setPasswordSettings(false)} />}<section className="panel real-trading-panel" hidden={tradingMode !== 'real'} aria-labelledby="real-title"><p className="eyebrow">Real trading</p><h1 id="real-title">Real trading is not available</h1><p>Live execution has not been enabled. No real orders can be sent from this workspace.</p><p className="muted space-top">This release has no wallet connection, signing or live order submission. Switching workspaces never changes the mode of an existing session.</p><button className="primary space-top" disabled>Start real trading</button></section><div hidden={tradingMode !== 'paper'}><div className="demo connected-banner"><strong>PAPER TRADING</strong><span>Research data and hypothetical accounting only. Automatic paper fills are not enabled. Closing this page does not stop workers.</span></div>
        {unresolvedRequest && <p id="unresolved-mode-note" className="notice">An unresolved request is retained in this Paper trading workspace. Mode switching is disabled until its outcome is reconciled. Reloading or closing the page discards the local retry key; inspect authoritative records before creating equivalent work.</p>}<div className="pagehead"><div><p className="eyebrow">Research / {page}</p><h1 ref={title} tabIndex={-1}>{page === 'Overview' ? 'Paper trading overview' : page}</h1><p className="subtitle">Inspect evidence and control each immutable research session.</p></div><div className="filter"><label htmlFor="connected-chain">View chain</label><select id="connected-chain" value={filter} onChange={event => setFilter(event.target.value as Network | 'all')} aria-describedby="connected-filter-note"><option value="all">All chains</option><option value="solana-mainnet">Solana</option><option value="base-mainnet">Base</option></select><p className="scope-note" id="connected-filter-note">View filter only. Stop controls retain their explicitly named session scope.</p></div></div>
        ...
        {snapshot && <><section className="controlbar" aria-label="Connected session control summary"><div><strong><span className={`pill ${stale ? 'amber' : 'blue'}`}>{stale ? 'STALE / LAST KNOWN' : 'API CONNECTED'}</span></strong><p className="statustext">Snapshot received {new Date(snapshot.at).toISOString()}. {allSessions.length} sessions loaded{snapshot.sessions.next_cursor ? ' · additional sessions are outside this page' : ''}.</p><p className="tiny">These controls target registered sessions. Separately launched capture processes have their own lifecycle. API connectivity does not establish feed freshness or worker health.</p><div className="session-states" aria-label="All loaded session command results">{Object.entries(commands).map(([id, command]) => <span key={id} className="session-status">{id}: {command.body.action} · {commandSummary(command)}</span>)}</div></div><div className="controls"><button disabled={!authenticated || busy || !stoppable.length} onClick={() => { void Promise.allSettled(stoppable.map(s => send(s, 'STOP'))); }}>Stop {snapshot.sessions.next_cursor ? 'loaded' : 'all loaded'} sessions ({stoppable.length})</button><button disabled={!authenticated || refreshing} onClick={() => { void refreshTrigger.current(); }}>Refresh</button></div></section>
          ...
          <div hidden={page !== 'Opportunities'}><DecisionExplorer active={authenticated && tradingMode === 'paper' && page === 'Opportunities'} api={api} capabilities={snapshot.capabilities} sessions={allSessions} filter={filter} disabled={!authenticated || stale} canRetry={authenticated} onPendingChange={setCostAssessmentPending} /></div>
          <div hidden={page !== 'Runs'}><PaperWorkspace active={authenticated && tradingMode === 'paper' && page === 'Runs'} api={api} capabilities={snapshot.capabilities} sessions={allSessions} filter={filter} disabled={!authenticated || stale} canRetry={authenticated} onPendingChange={setPaperCreationPending} /></div>
          ...
          <div hidden={page !== 'System'}><AdapterSupport active={authenticated && tradingMode === 'paper' && page === 'System'} available={snapshot.capabilities.adapter_support === true} api={api} filter={filter} /><CollectionHealth active={authenticated && tradingMode === 'paper' && page === 'System'} api={api} capabilities={snapshot.capabilities} sessions={allSessions} filter={filter} /></div>
      ...
      </main></div></div><RecordedDialog opportunity={inspected} onClose={() => setInspected(null)} /></div></>;
}
