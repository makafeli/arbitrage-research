...
export function ConnectedApp({ light, onToggleTheme }: { light: boolean; onToggleTheme: () => void }) {
  ...
  return <>{!authenticated && <AccountAccess api={api} checking={authLoading} light={light} onToggleTheme={onToggleTheme} />}<div hidden={!authenticated}><a className="skip" href="#main">Skip to content</a><div className="shell">
    <aside className="sidebar"><div className="brand"><span className="brandmark" aria-hidden="true">↗</span>Arbitrage</div><div className="subbrand">TRADING WORKSPACE</div><nav className="trading-modes" aria-label="Trading mode"><button aria-pressed={tradingMode === 'paper'} disabled={unresolvedRequest} onClick={() => setTradingMode('paper')}>Paper trading</button><button aria-pressed={tradingMode === 'real'} disabled={unresolvedRequest} onClick={() => { setInspected(null); setTradingMode('real'); }}>Real trading</button></nav><p className="navlabel">{tradingMode === 'paper' ? 'Paper workspace' : 'Real workspace'}</p><nav className="nav" aria-label="Primary navigation" hidden={tradingMode !== 'paper'}>{pages.map(name => <button key={name} aria-current={page === name ? 'page' : undefined} onClick={() => navigate(name)}>{name}</button>)}</nav><div className="sidefoot"><strong>Research first</strong>Persisted service evidence.<br />Dataset origins stay explicit.</div></aside>
