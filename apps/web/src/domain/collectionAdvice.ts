...
// (main.rs:520-521, 574, 611) — every other RESOURCE_LIMIT producer (the transport's
// request/byte budget at main.rs:131/463-469, the bundle transcript cap at main.rs:481, and managed_ingestion.rs:126-140's
...
const CAPTURE_VOLUME_FULL_ADVICE = 'Capture volume full: the capture store reached its byte quota (or one capture exceeded its byte quota). The retention prune frees space automatically on the next capture; if this repeats, review the capture quota and pool count with the operator. Do not retry manually.';
const RESOURCE_LIMIT_ACQUISITION_ADVICE = "Resource limit during acquisition: the RPC request/byte budget, a capture bundle's size, or the capture volume quota was exceeded. If the session is now FAULTED (worker log / session audit: MANAGED_SOURCE_RECOVERY_FAILED), the managed source hit its log limit and was halted — owner action required. Otherwise the retention prune runs before the next capture; if it repeats, review the capture quota and pool count with the operator.";
