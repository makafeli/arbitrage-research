...
export function PaperWorkspace({ active, api, capabilities, sessions, filter, disabled, canRetry, onPendingChange }: Props) {
  ...
  return <section className="section-spacer research-workspace" aria-labelledby="paper-workspace-title">
    ...
    {!available && !creationLocked ? <EmptyResearch>Paper accounting is unavailable in this API version. No balances or performance estimates have been invented.</EmptyResearch> : <>
      <div className="panel research-toolbar"><label htmlFor="paper-session">Paper session<select id="paper-session" value={sessionId} disabled={creationLocked} onChange={event => { setChosen(sessions.find(session => session.session_id === event.target.value) ?? null); setRunId(''); }}><option value="">Choose a PAPER session</option>{options.map(session => <option key={session.session_id} value={session.session_id}>{session.session_id} · {session.network_id} · {session.observed_state}</option>)}</select></label><button disabled={!enabled || runs.loading || run.loading || journal.loading || reservations.loading} onClick={refresh}>Refresh paper records</button></div>
