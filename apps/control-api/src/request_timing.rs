...
async fn request_timing_is_correlated_bounded_and_does_not_change_authentication() {
    ...
    let app = router(AppState::new(store, ServerConfig {
    ...
    let request = Request::builder().uri("/v1/capabilities").body(Body::empty()).unwrap();
    ...
    let login = app.clone().oneshot(Request::builder()
        .method("POST").uri("/v1/auth/login")
        .header("Origin", origin).header("Content-Type", "application/json")
        .body(Body::from(serde_json::json!({"operator_secret": secret}).to_string())).unwrap()
    ).await.unwrap();
    assert_eq!(login.status(), StatusCode::OK);
    check_headers(&login);
    let cookie = login.headers()["set-cookie"].to_str().unwrap().split(';').next().unwrap().to_string();
    let allowed = app.oneshot(Request::builder().uri("/v1/capabilities")
...
fn check_headers(response: &Response) {
    ...
    assert!(response.headers()["x-request-id"].to_str().unwrap().starts_with("req-"));
}
