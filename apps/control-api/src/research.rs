...
pub(crate) trait ResearchStore: Send + Sync {
    async fn list_decisions(&self, operator: &str, session: &str, cursor: Option<&str>, limit: u32) -> Result<DecisionTracePage, StoreError>;
    async fn get_decision(&self, operator: &str, observation: &str) -> Result<StoredDecisionTrace, StoreError>;
    async fn decision_groups(&self, operator: &str, session: &str, cursor: Option<&str>, limit: u32) -> Result<DecisionGroupPage, StoreError>;
    async fn decision_coverage(&self, operator: &str, session: &str) -> Result<DecisionCoverage, StoreError>;
    async fn opportunities(&self, operator: &str, filter: OpportunityFilter, cursor: Option<&str>, limit: u32) -> Result<OpportunityPage, StoreError>;
    async fn replay_paper_creation(&self, operator: &str, session: &str, key: &str, input: &NewPaperRun) -> Result<Option<PaperRunRecord>, StoreError>;
    async fn create_paper_run(&self, operator: &str, session: &str, key: &str, input: NewPaperRun) -> Result<PaperRunRecord, StoreError>;
    async fn get_paper_run(&self, operator: &str, run: &str) -> Result<PaperRunRecord, StoreError>;
    async fn list_paper_runs(&self, operator: &str, session: &str, cursor: Option<&str>, limit: u32) -> Result<PaperRunPage, StoreError>;
    async fn paper_journal(&self, operator: &str, run: &str, cursor: Option<&str>, limit: u32) -> Result<PaperJournalPage, StoreError>;
    async fn paper_reservations(&self, operator: &str, run: &str, cursor: Option<&str>, limit: u32) -> Result<PaperReservationPage, StoreError>;
}
...
fn session_page(query: &SessionQuery, id: &RequestId) -> Result<u32, ApiError> {
    ...
    validate_page(&Pagination { cursor: query.cursor.clone(), limit: query.limit }, id)
}
pub(super) async fn list_decisions(
    ...
    state.0.store.list_decisions("operator", &query.session_id, query.cursor.as_deref(), limit)
        .await.map(Json).map_err(|error| ApiError::store(error, &id))
}
pub(super) async fn get_decision(
    ...
    state.0.store.get_decision("operator", &observation)
        .await.map(Json).map_err(|error| ApiError::store(error, &id))
}
pub(super) async fn decision_groups(
    ...
    let Query(query) = query.map_err(|_| ApiError::invalid(&id))?;
    let limit = session_page(&query, &id)?;
    state.0.store.decision_groups("operator", &query.session_id, query.cursor.as_deref(), limit)
        .await.map(Json).map_err(|error| ApiError::store(error, &id))
}
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
pub(super) struct CoverageQuery { session_id: String }
pub(super) async fn decision_coverage(
    ...
    state.0.store.decision_coverage("operator", &query.session_id)
        .await.map(Json).map_err(|error| ApiError::store(error, &id))
}
pub(super) async fn create_paper_run(
    ...
    if let Some(existing) = state.0.store.replay_paper_creation("operator", &session_id, &key, &input)
        .await.map_err(|error| ApiError::store(error, &id))? {
        return Ok((StatusCode::CREATED, Json(existing)));
    }
    let session = state.0.store.get_session("operator", &session_id)
        .await.map_err(|error| ApiError::store(error, &id))?;
    if session.mode != "PAPER" {
        return Err(ApiError::store(StoreError::CapabilityUnavailable, &id));
    }
    let config = state.0.config.configurations.iter()
        .find(|config| config.configuration_digest == session.configuration_digest)
        .ok_or_else(|| ApiError::new(StatusCode::BAD_REQUEST, "UNKNOWN_CONFIGURATION", "Configuration digest is not registered", &id))?;
    if config.mode != "PAPER" || !config.enabled_networks.contains(&session.network_id)
        || input.initial_balances.is_empty() || input.initial_balances.len() > 32 {
        return Err(ApiError::invalid(&id));
    }
    let mut seen = std::collections::HashSet::new();
    for balance in &input.initial_balances {
        if !seen.insert(balance.asset.clone())
            || !config.paper_assets.iter().any(|choice| choice.network_id == session.network_id && choice.asset == balance.asset) {
            return Err(ApiError::invalid(&id));
        }
    }
    ...
    state.0.store.create_paper_run("operator", &session_id, &key, input).await
        .map(|run| (StatusCode::CREATED, Json(run))).map_err(|error| ApiError::store(error, &id))
}
pub(super) async fn get_paper_run(
    ...
    state.0.store.get_paper_run("operator", &run)
        .await.map(Json).map_err(|error| ApiError::store(error, &id))
}
pub(super) async fn list_paper_runs(
    ...
    state.0.store.list_paper_runs("operator", &session, query.cursor.as_deref(), limit)
        .await.map(Json).map_err(|error| ApiError::store(error, &id))
}
pub(super) async fn paper_journal(
    ...
    let Query(query) = query.map_err(|_| ApiError::invalid(&id))?;
    let limit = validate_page(&query, &id)?;
    state.0.store.paper_journal("operator", &run, query.cursor.as_deref(), limit)
        .await.map(Json).map_err(|error| ApiError::store(error, &id))
}
pub(super) async fn paper_reservations(
    ...
    let Query(query) = query.map_err(|_| ApiError::invalid(&id))?;
    let limit = validate_page(&query, &id)?;
    state.0.store.paper_reservations("operator", &run, query.cursor.as_deref(), limit)
        .await.map(Json).map_err(|error| ApiError::store(error, &id))
}
