...
async fn research_routes_require_auth_and_keep_mutation_guards() {
    ...
    for (origin, token) in [
        ...
        let response = call(
            ...
            Method::POST,
            "/v1/sessions/x/paper-runs",
            Some(json!({"initial_balances":[]})),
            Some(&cookie),
            token,
            origin,
            Some("paper-create-key-01"),
        )
        .await;
        assert_eq!(response.status(), StatusCode::FORBIDDEN);
    }
    assert_eq!(store.calls.load(Ordering::SeqCst), 0);
    for path in [
        "/v1/paper-runs/x/settle",
        "/v1/paper-runs/x/reserve",
        "/v1/decisions/x/realize",
    ] {
        let response = call(
            &app,
            Method::POST,
...
async fn research_page_and_paper_payload_bounds_fail_before_read_queries() {
    ...
    for path in [
        "/v1/decisions",
        "/v1/decisions?session_id=x&limit=101",
    ...
    assert_eq!(caps["live_execution"], false);
    assert_eq!(
        caps["registered_configurations"][0]["paper_assets"],
        json!([])
    );
}
...
async fn postgres_paper_http_immutable_runs_survive_restart_and_enforce_scope() {
    ...
    let digest = format!("sha256:{unique}");
    let asset: arb_domain::AssetId = "base-mainnet:0x1111111111111111111111111111111111111111"
        .parse()
        .unwrap();
    let snapshot = json!({"deployment":{"mode":"PAPER"}, "networks":{"base":{"enabled":true,"verified_asset_ids":[asset.to_string()]}}});
    store
        .save_configuration("operator", &digest, snapshot.clone())
        .await
        .unwrap();
    let session = store
        .create_session(
            "operator",
            &format!("paper-session-{unique}"),
            NewSession {
                network_id: "base-mainnet".into(),
                mode: "PAPER".into(),
                configuration_digest: digest.clone(),
                experiment_id: format!("paper-http-{unique}"),
                strategy_ids: vec!["cyclic-exact-in-2leg-v1".into()],
            },
        )
        .await
    ...
    let configuration = RegisteredConfiguration {
        ...
        paper_assets: vec![
            PaperAssetChoice {
                network_id: "base-mainnet".into(),
                asset: arb_paper::AccountingAsset::Token(asset.clone()),
            },
            PaperAssetChoice {
                network_id: "base-mainnet".into(),
                asset: arb_paper::AccountingAsset::Native(arb_domain::NetworkId::BaseMainnet),
            },
        ],
    ...
    changed["initial_balances"][0]["amount"] = json!("200000000");
    assert_eq!(
        call(
            &app,
            Method::POST,
    ...
    forbidden["initial_balances"][0]["asset"]["identity"] =
        json!("base-mainnet:0x2222222222222222222222222222222222222222");
    assert_eq!(
        call(
            &app,
            Method::POST,
    ...
    let cursor = page["next_cursor"]
        .as_str()
        .expect("two runs require another page");
    let next = value(
        call(
            &app,
            Method::GET,
    ...
    let journal = value(
        call(
            &app,
            Method::GET,
            &format!("/v1/paper-runs/{run_id}/journal"),
            None,
            Some(&cookie),
            None,
            None,
    ...
    assert_eq!(
        journal["items"][0]["event"]["command"]["kind"],
        "INITIALIZE"
    );
    ...
    let reservations = value(
        call(
            &app,
            Method::GET,
            &format!("/v1/paper-runs/{run_id}/reservations"),
            None,
            Some(&cookie),
            None,
            None,
    ...
    store
        .save_configuration(&other, &digest, snapshot)
        .await
        .unwrap();
    let hidden_session = store
        .create_session(
            &other,
            &format!("hidden-{unique}"),
            NewSession {
                network_id: "base-mainnet".into(),
                mode: "PAPER".into(),
                configuration_digest: digest,
                experiment_id: format!("hidden-{unique}"),
                strategy_ids: vec!["cyclic-exact-in-2leg-v1".into()],
            },
        )
        .await
        .unwrap();
    let hidden_worker = arb_control::ControlWorker::claim(
        store.clone(),
        &other,
        &hidden_session.session_id,
        "base-mainnet",
        &format!("hidden-worker-{unique}"),
        60,
    )
    .await
    .unwrap();
    ...
    assert_eq!(
        call(
            ...
            Some(&new_csrf),
            Some(ORIGIN),
            Some(&format!("unregistered-{unique}"))
        )
        .await
        .status(),
        StatusCode::BAD_REQUEST
...
fn http_trace_fixture(
    session: &SessionRecord,
    experiment: &str,
    generation: u64,
) -> arb_domain::DecisionTrace {
    use arb_domain::*;
    let a = AssetId::new(
...
async fn postgres_decision_http_counts_rejections_without_undercounting_eligible_pages() {
    ...
    store
        .save_configuration(
            "operator",
            &digest,
            json!({"fixture":true,"deployment":{"mode":"OBSERVE"}}),
        )
        .await
        .unwrap();
    let session = store
    ...
    for capture in &quote.capture_refs {
        worker
            .admit_capture_manifest(
                work,
                &capture.capture_id,
                &capture.manifest_digest,
                "/synthetic-http-fixture/manifest.json",
            )
            .await
            .unwrap();
    }
    let mut zero = quote.clone();
    zero.observed_at_unix_ms += 1;
    zero.result = DecisionResult::Quoted {
        quoted_output_minor: 100_u64.into(),
        gross_delta_minor: "0".parse().unwrap(),
        included_pool_fees: vec![1_u64.into(), 1_u64.into()],
    };
    let zero = zero.seal().unwrap();
    assert_eq!(quote.grouping.key, zero.grouping.key);
    let mut rejected = quote.clone();
    rejected.observed_at_unix_ms += 2;
    rejected.result = DecisionResult::Rejected {
        reason_codes: vec!["MATH_INPUT_REJECTED".into()],
    };
    let rejected = rejected.seal().unwrap();
    let mut no_route = quote.clone();
    no_route.observed_at_unix_ms += 3;
    no_route.route.clear();
    no_route.amount_in_minor = None;
    no_route.result = DecisionResult::NoRoute {
        reason_codes: vec!["NO_ELIGIBLE_POOL_PAIRS".into()],
    };
    let no_route = no_route.seal().unwrap();
    let mut unavailable = quote.clone();
    unavailable.observed_at_unix_ms += 4;
    unavailable.route.clear();
    unavailable.amount_in_minor = None;
    unavailable.input_age_ms = None;
    unavailable.capture_refs.clear();
    unavailable.result = DecisionResult::DataUnavailable {
        reason_codes: vec!["NO_CAPTURE_INPUTS".into()],
    };
    ...
    assert_eq!(
        accepted.iter().map(|r| &r.trace_id).collect::<Vec<_>>(),
        repeated.iter().map(|r| &r.trace_id).collect::<Vec<_>>()
    );
    let coverage = value(
        call(
            &app,
    ...
    for _ in 0..8 {
        let path = format!(
            "/v1/opportunities?session_id={}&source_kind=SYNTHETIC_FIXTURE&limit=1{}",
            session.session_id,
            cursor
                .as_ref()
        ...
        assert_eq!(
            ...
            1,
            "eligible paging cannot produce empty intermediate pages"
        );
    ...
    assert!(
        cursor.is_none(),
        "bounded fixture pagination must terminate"
    );
    assert_eq!(
        seen,
        std::collections::HashSet::from([
            quote.observation_id.clone(),
            zero.observation_id.clone()
        ])
    );
    for filter in [
        "source_kind=CAPTURED_MARKET_DATA",
        ...
        let filtered = value(
            call(
                ...
                Method::GET,
                &format!(
                    "/v1/opportunities?session_id={}&{filter}",
                    session.session_id
                ),
                None,
                Some(&cookie),
    ...
    let detail = value(
        call(
            &app,
            Method::GET,
            &format!("/v1/decisions/{}", rejected.observation_id),
            None,
            Some(&cookie),
            None,
            None,
    ...
    let mut trace_cursor = None::<String>;
    let mut observations = std::collections::HashSet::new();
    for _ in 0..8 {
        let path = format!(
            "/v1/decisions?session_id={}&limit=2{}",
            session.session_id,
            trace_cursor
                .as_ref()
                .map(|c| format!("&cursor={c}"))
        ...
        for item in page["items"].as_array().unwrap() {
            assert!(
                observations.insert(item["trace"]["observation_id"].as_str().unwrap().to_owned())
            );
        }
        trace_cursor = page["next_cursor"].as_str().map(str::to_owned);
        if trace_cursor.is_none() {
            break;
        }
    }
    ...
    let groups = value(
        call(
            ...
            Method::GET,
            &format!(
                "/v1/decision-groups?session_id={}&limit=1",
                session.session_id
            ),
            None,
            Some(&cookie),
    ...
    store
        .save_configuration(&other, &digest, json!({"fixture":true}))
        .await
        .unwrap();
    let hidden_session = store
        .create_session(
            &other,
            &format!("hidden-trace-{unique}"),
            NewSession {
                network_id: "base-mainnet".into(),
                mode: "OBSERVE".into(),
                configuration_digest: digest,
    ...
    let hidden_worker = arb_control::ControlWorker::claim(
        ...
        &hidden_session.session_id,
        "base-mainnet",
        &format!("hidden-trace-worker-{unique}"),
        60,
    )
    .await
    .unwrap();
    ...
    let mut hidden = unavailable;
    hidden.session_id = hidden_session.session_id.clone();
    hidden.generation = hidden_work.generation();
    let hidden = hidden.seal().unwrap();
    hidden_worker
        .admit_decision_traces(hidden_work, std::slice::from_ref(&hidden))
        .await
        .unwrap();
    for path in [
        ...
        format!(
            "/v1/decision-groups?session_id={}",
            hidden_session.session_id
        ),
        format!(
            "/v1/decision-coverage?session_id={}",
            hidden_session.session_id
        ),
...
fn opportunity_examples_preserve_legacy_and_explicit_unknown_net() {
    ...
    let record: arb_domain::OpportunityRecord =
        serde_json::from_str(include_str!("../../../specs/opportunity.v1.1.example.json")).unwrap();
