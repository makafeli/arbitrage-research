...
use arb_evm::{PoolRegistry, SOURCE_COMMIT, capture_pool};
use std::{
    error::Error,
    fs,
    path::Path,
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    if args.first().is_none_or(|x| x != "record" && x != "fixture") || args.len() != 4 {
        return Err("Usage: evm-worker record CONFIG.toml REGISTRY.json NEW_CAPTURE_DIRECTORY | fixture REGISTRY.json TRANSCRIPT.json NEW_CAPTURE_DIRECTORY | inspect CAPTURE_DIRECTORY [EXPECTED_MANIFEST_SHA256]. One-shot read-only command, independent of dashboard session controls.".into());
    }
    ...
    let (snapshot, records, origin, config_digest, config_bytes, provider, quota, retention_days) =
        if synthetic {
            ...
            if !config
                ...
                .iter()
                .any(|p| p.address().eq_ignore_ascii_case(&registry.pool))
            {
                return Err("pool is not in configured allowlist".into());
            }
            for address in [registry.token0.as_str(), registry.token1.as_str()] {
                if !config
    ...
    let manifest = CaptureManifest {
        ...
        adapter_version: "arb_evm-v1".into(),
