# Solana worker: one-shot capture CLI
...
```sh
cargo run -p solana-worker -- fixture \
  crates/arb-solana/tests/fixtures/registry.json \
  crates/arb-solana/tests/fixtures/rpc.json \
  /tmp/solana-manual-capture

cargo run -p solana-worker -- inspect /tmp/solana-manual-capture
```
...
```sh
cargo run -p solana-worker -- record /path/to/research.toml /path/to/qualified-registry.json /path/to/new-capture
```
...
See [adapter documentation](../../crates/arb-solana/README.md) and [bundle format](../../crates/arb-capture/README.md).
