...
use super::*;
use arb_storage::{IngestionBinding, IngestionHalt, IngestionHead};
use sqlx::PgPool;
...
async fn source_process(matching: bool) {
    ...
    let document =
        arb_registry::RegistryDocument::from_bytes(&registry, arb_domain::NetworkId::BaseMainnet)
            .unwrap();
    let pools: Vec<_> = document
        .pools()
        .iter()
        .map(|p| match p {
            arb_registry::PoolRegistry::Base(p) => p.clone(),
            _ => panic!("Base fixture required"),
        })
        .collect();
    ...
    let arb_adapter_api::StateContext::Evm {
        block_number,
        block_hash,
        parent_hash,
    ...
    let seed = IngestionHead {
        number: block_number - 1,
        hash: checkpoint.parent_hash.clone(),
        parent_hash: format!("0x{}", "ee".repeat(32)),
        timestamp_seconds: block_timestamp_seconds.saturating_sub(1),
    };
    ...
    let initial = store
        .create_ingestion(&operator, "source", &binding, &seed)
        .await
        .unwrap();
    ...
    let id = store
        .create_session(
            ...
            NewSession {
                ...
                experiment_id: "synthetic-source-process".into(),
    ...
    if matching {
        ...
        assert!(tracked >= 2);
        store
            .halt_ingestion(&operator, "source", &cursor, IngestionHalt::ContinuityLost)
            .await
            .unwrap();
    }
    ...
    let status = loop {
        ...
        assert!(
            ...
            "worker did not stop after invalid source: {}",
            fs::read_to_string(root.join("worker.log")).unwrap()
        );
