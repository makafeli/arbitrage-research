...
The process tests cover registration before held RPC I/O, redacted HTTP 503 failure, a killed unfinished capture, responsive STOP with a suppressed late capture, and individually valid pool captures whose differing block contexts produce durable route rejections without quoted opportunities. The two-pool end-to-end test checks that one completed collection owns exactly two persisted decisions and two captured inputs. All inputs in these tests are synthetic loopback fixtures.
