...
`replay --verify-capture DIRECTORY --manifest-digest sha256:HASH` verifies retained capture hashes, exact request/response order, protocol source revision, frozen configuration authorization where required, and deterministic re-decoding. It makes zero network requests. The original single-pool capture format and historical `arb_evm-pool-set-v1` / `arb_solana-pool-set-v1` captures remain supported. Pool-set-v1 replays its selected pool's original independent transcript. New `*-pool-set-v2` captures replay **all pools** from the exact retained registry document against the complete actual shared batch transcript before selecting `snapshot.json.pool`. Base uses one anchor and final canonical check; Solana uses one union account response. No synthetic per-pool RPC responses are generated. A corrupt unselected member, omitted or trailing RPC record, wrong selected snapshot, or v1 transcript relabelled v2 fails verification. Repeated per-pool bundles may retain the same full batch bytes; all are still subject to normal object, byte, origin, configuration, hash and expiry checks.
...
For either network, a present `chain_freshness` field requires the complete frozen configuration to validate. A null or malformed policy cannot fall back to legacy replay; policy absence requires an omitted field. Legacy decoder-only fixtures that omit policy fields remain supported.
