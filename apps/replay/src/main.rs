...
fn main() -> ExitCode {
    ...
    if args.len() == 2 && args[0] == "--evaluate-captures" {
        return match evaluate_file(&args[1]) {
            ...
            Err(_) => {
                eprintln!("Replay evaluation failed: invalid request, unavailable retained inputs or incompatible evidence");
                ExitCode::FAILURE
            }
        };
    }
    if args.len() != 1 || args[0] != "--lifecycle-demo" {
        ...
        eprintln!("Research arithmetic replay produces CANDIDATE evidence only; paper-outcome replay is unavailable.");
...
fn evaluate_file(path: &str) -> Result<serde_json::Value, Box<dyn Error>> {
    ...
    std::fs::File::open(path)?.take(1024*1024+1).read_to_end(&mut bytes)?;
    if bytes.len() > 1024*1024 { return Err("request exceeds byte bound".into()); }
    let request: replay::ReplayEvaluationRequest = serde_json::from_slice(&bytes)?;
    let now = u64::try_from(std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)?.as_millis())?;
    let mut report = replay::evaluate_captures(&request, now)?;
    report["replay_build_digest"] = serde_json::json!(arb_capture::file_digest(&std::env::current_exe()?)?);
    Ok(report)
}
