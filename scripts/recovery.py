    ...
    with source_file(root, path) as source:
        ...
        try:
            ...
            while chunk := source.read(1024 * 1024):
                size += len(chunk)
                require(size <= limit, 'Artifact byte bound exceeded')
                h.update(chunk)
...
def inventory(root, limits, omit=()):
    ...
    def walk(fd, prefix):
        ...
        with os.scandir(fd) as stream:
            ...
            for entry in stream:
                entries += 1
                require(entries <= limits.files * 4 + 32, 'Directory entry bound exceeded')
        ...
        for name in sorted(names):
            ...
            if stat.S_ISDIR(info.st_mode):
                ...
                if path not in omit:
                    paths.append(path)
                    require(len(paths) <= limits.files, 'Artifact count bound exceeded')
    fd = os.open(root, os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW)
    try:
        walk(fd, '')
    finally:
...
class Diagnostics:
    """Private, operation-scoped stderr storage. Never part of a backup manifest."""

    max_commands = 4096
    max_file_bytes = 1024**2
    max_total_bytes = 4 * 1024**2

    def __init__(self, operation, path=None):
        require(operation in {'backup', 'restore', 'command'}, 'Invalid diagnostic operation')
        self.identifier = uuid.uuid4().hex
        if path is None:
            parent = Path(tempfile.gettempdir()).resolve(strict=True)
            path = parent / ('arb-recovery-' + operation + '-' + self.identifier)
        self.root = Path(path).absolute()
        root_path(self.root.parent)
        require(not self.root.exists() and not self.root.is_symlink(), 'Diagnostic destination must be new')
    ...
    def exclude(self, *paths):
        for path in paths:
            if path is not None:
                ...
                require(not self.root.is_relative_to(protected)
                        and not protected.is_relative_to(self.root),
                        'Diagnostic storage must be separate from all source and artifact roots')

    def begin(self):
        if not self.created:
            new_directory(self.root)
            write_private(self.root / 'operation.json', canonical({
                'schema_version': 1, 'operation': self.operation, 'diagnostics_id': self.identifier,
            }))
            self.created = True
        require(self.sequence < self.max_commands, 'Diagnostic command count bound exceeded')
        self.sequence += 1
...
def command(args, limits, *, capture=False, readonly=False, diagnostics=None, input_file=None):
    ...
    h = hashlib.sha256()
    size = rows = stderr_seen = stderr_kept = 0
    chunks = []
    ...
    with os.fdopen(os.open(stderr_path, os.O_WRONLY | os.O_CREAT | os.O_EXCL | os.O_NOFOLLOW, 0o600), 'wb') as error_output:
        try:
                    ...
                    while selector.get_map():
                        ...
                        for key, _ in selector.select(0.1):
                            ...
                            if not data:
                                ...
                                continue
                            if key.data == 'stderr':
                                stderr_seen += len(data)
                                retain = max(0, min(len(data), store.max_file_bytes - stderr_kept,
                                                    store.max_total_bytes - store.retained_bytes))
                                if retain:
                                    error_output.write(data[:retain])
                                    stderr_kept += retain
                                    store.retained_bytes += retain
                                # Continue draining after the storage cap to avoid pipe deadlock.
                                continue
                            size += len(data)
                            rows += data.count(b'\n')
                            h.update(data)
                            ...
                            if capture:
                                chunks.append(data)
                    returncode = process.wait(timeout=max(0.1, limits.seconds - (time.monotonic() - started)))
                    require(returncode == 0, 'PostgreSQL command failed; inspect private diagnostics')
                    completed = True
                finally:
                    selector.close()
                    if not completed:
            ...
            # No argv, connection details, SQL or raw stderr is included in the metadata.
            write_private(stem.with_suffix('.json'), canonical({
                'schema_version': 1, 'completed': completed, 'returncode': returncode,
                'stderr_bytes_seen': stderr_seen, 'stderr_bytes_retained': stderr_kept,
                'stderr_truncated': stderr_seen > stderr_kept,
            }))
...
def sql(database, statement, limits, *, capture=True, diagnostics=None):
    return command(['psql', '-X', '-qAt', '-w', '-v', 'ON_ERROR_STOP=1', '--dbname', database_name(database), '-c', statement],
                   limits, capture=capture, readonly=True, diagnostics=diagnostics)
...
def db_evidence(database, limits, *, diagnostics=None):
    ...
    for relation in relations:
        ...
        records.append({**relation, **sql(database, statement, limits, capture=False, diagnostics=diagnostics)})
...
def backup(database, captures, output, source_commit, quiesced, limits=Limits(), *, diagnostics=None):
    ...
    diagnostics = diagnostics if diagnostics is not None else Diagnostics('backup')
    diagnostics.exclude(captures, output)
    version = int(sql(database, 'SHOW server_version_num', limits, diagnostics=diagnostics)) // 10000
    require(version >= 15, 'PostgreSQL 15 or newer required')
    before_files = inventory(captures, limits)
    before_db = db_evidence(database, limits, diagnostics=diagnostics)
    output = new_directory(output)
    # Incomplete output is retained without a final manifest, never passed off as a backup.
    command(['pg_dump', '-w', '--format=custom', '--dbname', database, '--file', str(output / 'database.dump')], limits, readonly=True, diagnostics=diagnostics)
    ...
    require(before_db == db_evidence(database, limits, diagnostics=diagnostics), 'Database changed during quiesced backup')
    ...
    require(len(raw) <= 16 * 1024**2, 'Manifest too large')
    write_private(output / 'manifest.json', raw)
    return {'status': 'BACKUP_COMPLETE', 'manifest_sha256': digest(raw), 'files': len(records), 'source_commit': source_commit, 'diagnostics_id': diagnostics.identifier}
...
def verify(bundle, expected, limits=Limits()):
    ...
    manifest, raw = read_json(bundle, 'manifest.json')
    ...
    require(isinstance(records, list) and 2 <= len(records) <= limits.files, 'Invalid artifact list')
    names = []
    for record in records:
...
def restore(bundle, expected, database, confirm_target, destination, trusted_source, limits=Limits(), *, diagnostics=None):
    ...
    diagnostics = diagnostics if diagnostics is not None else Diagnostics('restore')
    diagnostics.exclude(bundle, destination)
    require(int(sql(database, 'SHOW server_version_num', limits, diagnostics=diagnostics)) // 10000 == manifest['postgres_major'], 'Restore drill requires the same PostgreSQL major')
    ...
    require(int(sql(database, nonempty, limits, diagnostics=diagnostics)) == 0, 'Restore database must be empty and isolated')
    ...
    return {'status': 'ISOLATED_RESTORE_VERIFIED', 'manifest_sha256': expected, 'relations': len(evidence['relations']),
            ...
            'diagnostics_id': diagnostics.identifier}
...
def main():
    ...
    parser.add_argument('--diagnostics', help='New private diagnostic directory, outside bundle/capture roots')
    ...
    try:
        ...
        if args.operation == 'backup':
            ...
            diagnostics = Diagnostics('backup', args.diagnostics)
            result = backup(args.database, args.captures, args.output, args.source_commit, args.quiesced, limits, diagnostics=diagnostics)
        else:
            ...
            if args.operation == 'verify':
                ...
                diagnostics = Diagnostics('restore', args.diagnostics)
                result = restore(args.bundle, args.manifest_sha256, args.database, args.confirm_target, args.captures, args.trusted_source, limits, diagnostics=diagnostics)
        ...
        print(json.dumps({'status': 'RECOVERY_FAILED', 'message': 'Precondition, command or verification failed. Keep partial output/target quarantined; inspect private diagnostics and consult the recovery runbook.',
                          'diagnostics_id': diagnostics.identifier if diagnostics is not None else None}))
        return 2
